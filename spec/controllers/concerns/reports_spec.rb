...
describe ReportsSpec do
  ...
  it "creates header row properly" do
    ...
    expect(csv_contents[0][6]).to eq "uid"
    expect(csv_contents[0][7]).to eq "is_eligible"
    expect(csv_contents[0][8]).to eq "first_name"
    expect(csv_contents[0][9]).to eq "last_name"
    expect(csv_contents[0][10]).to eq "email"
    expect(csv_contents[0][11]).to eq "gender"
    expect(csv_contents[0][12]).to eq "age"
    expect(csv_contents[0][13]).to eq "zip_code"
    expect(csv_contents[0][14]).to eq "is_hispanic_or_latino"
  ...
  def check_common_columns(csv_contents, users)
    ...
    expect(csv_contents[1][2]).to eq "0" #"test_user"
    expect(csv_contents[1][3]).to eq "0" #"hancock-health-improvement-partnership"
    expect(csv_contents[1][4]).to eq "0" #"mercy-health-center"

    #using user_has_completed_food_etalk causes issues here...
    expect(csv_contents[1][5]).to eq users.first.created_at.to_s #"signup_date"

    expect(csv_contents[1][6]).to eq "uid|1" #"uid"
    expect(csv_contents[1][7]).to eq "1" #"is_eligible"
    expect(csv_contents[1][8]).to eq "Test" #"first_name"
    expect(csv_contents[1][9]).to eq "User" #"last_name"
    expect(csv_contents[1][10]).to eq "tester1@example.com" #"email"
    expect(csv_contents[1][11]).to eq "male" #"gender"
    expect(csv_contents[1][12]).to eq "21" #"age"
    expect(csv_contents[1][13]).to eq "30601" #"zip_code"
    expect(csv_contents[1][14]).to eq "false" #"is_hispanic_or_latino"
  end

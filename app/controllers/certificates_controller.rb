  ...
  def show
    ...
    begin
      ...
      when LearningModules.module_name(:BETTER_U)
        ...
        food_etalk_completion_date = curriculum_completion_date(@current_user, LearningModules::FOOD_ETALK).strftime("%B %d, %Y")

        better_u_image_name = 'BetterUCompletionCertificate.png'
        better_u_completion_date = curriculum_completion_date(@current_user, LearningModules::BETTER_U).strftime("%B %d, %Y")
